         ...
         ;以下常量定义部分。内核的大部分内容都应当固定 (选择子的RPL字段都为0)
...
read_hard_disk_0:                           ;从硬盘读取一个逻辑扇区
         ...
         mov dx,0x1f2
         mov al,1
         out dx,al                          ;读取的扇区数写入0x1f2

         inc dx                             ;0x1f3
         pop eax
         out dx,al                          ;LBA地址7~0写入0x1f3
...
allocate_memory:                            ;分配内存
         ...
         mov eax,[ram_alloc]
         add eax,ecx                        ;eax -> 下一次分配时的起始地址
      
         ;这里应当有检测可用内存数量的指令
          
         mov ecx,[ram_alloc]                ;ecx -> 返回分配的起始地址
...
set_up_gdt_descriptor:                      ;在GDT内安装一个新的描述符
         ...
         inc bx                             ;bx -> GDT总字节数，也是下一个描述符偏移
         add ebx,[pgdt+2]                   ;ebx -> 下一个描述符的线性地址(GDT基地址加界限)

         mov [es:ebx],eax                   ;装入描述符第32位
         mov [es:ebx+4],edx                 ;装入描述符高32位

         add word [pgdt],8                  ;增加一个描述符的大小（GDT界限加8）

         lgdt [pgdt]                        ;对GDT的更改生效

         mov ax,[pgdt]                      ;ax -> 得到GDT界限值
         xor dx,dx
         mov bx,8
         div bx                             ;除以8，去掉余数(ax -> 调用门描述符在GDT中的索引)
         mov cx,ax
         shl cx,3                           ;将索引号移到正确位置（返回cs -> 调用门选择子）
...
fill_descriptor_in_ldt:                     ;在LDT内安装一个新的描述符
         ...
         xor dx,dx
         mov cx,8
         div cx                             ;ax -> 除法的商，即为当前新描述符索引号
         
         mov cx,ax
         shl cx,3                           ;左移3位，并且
         or cx,0000_0000_0000_0100B         ;使TI位=1，指向LDT，最后使RPL=00 （cx -> 最终段选择子）
...
load_relocate_program:                      ;加载并重定位用户程序
         ...
         mov ecx,mem_0_4_gb_seg_sel
         mov es,ecx                         ;es -> 4GB内存段
      
         mov esi,[ebp+11*4]                 ;esi -> 从堆栈中取得TCB的基地址

         ;以下申请创建LDT所需要的内存
         mov ecx,160                        ;160个字节的空间用户创建LDT，允许安装20个LDT描述符
         call sys_routine_seg_sel:allocate_memory
         mov [es:esi+0x0c],ecx              ;登记LDT基地址到TCB中
         mov word [es:esi+0x0a],0xffff      ;登记LDT初始的界限到TCB中（刚创建LDT，总字节数为0，界限为
                                            ;0x0000-1）
         ;以下开始加载用户程序 
         mov eax,core_data_seg_sel
         ...
         mov ebx,core_buf                   ;读取程序头部数据(ebx -> 内核数据段缓冲区)
         call sys_routine_seg_sel:read_hard_disk_0

         ;以下判断整个程序有多大
         mov eax,[core_buf]                 ;程序尺寸
         mov ebx,eax                        ;ebx -> 程序尺寸
         and ebx,0xfffffe00                 ;使之512字节对齐（能被512整除的数低 
         add ebx,512                        ;9位都为0 
         test eax,0x000001ff                ;程序的大小正好是512的倍数吗? 
         cmovnz eax,ebx                     ;不是。使用凑整的结果
      
         mov ecx,eax                        ;ecx -> 实际需要申请的内存数量
         call sys_routine_seg_sel:allocate_memory
  ...
  .b1:
         ...
         mov edi,[es:esi+0x06]              ;edi -> 获得程序加载基地址

         ;建立程序头部段描述符
         mov eax,edi                        ;eax -> 程序头部起始线性地址
         mov ebx,[edi+0x04]                 ;ebx -> 段长度
         dec ebx                            ;ebx -> 段界限
         mov ecx,0x0040f200                 ;ecx -> 字节粒度的数据段描述符，特权级3 
         call sys_routine_seg_sel:make_seg_descriptor
      
         ;安装头部段描述符到LDT中 
         mov ebx,esi                        ;ebx -> TCB的基地址, edx:eax -> 头部段描述符
         call fill_descriptor_in_ldt        ;返回cx -> 新生成的LDT描述符选择子
         ...
         mov es,eax                         ;已安装，但还没有生效，故只能通
                                            ;过4GB段访问用户程序头部   es -> 0..4GB段
         mov eax,core_data_seg_sel
         mov ds,eax                         ;ds -> 内核数据段
      
         cld

         mov ecx,[es:edi+0x24]              ;ecx -> U-SALT条目数(通过访问4GB段取得) 
         add edi,0x28                       ;edi -> U-SALT在4GB段内的偏移 
  .b2: 
         push ecx
         push edi
      
         mov ecx,salt_items                 ;ecx -> C-SALT条目数
         mov esi,salt                       ;esi -> C-SALT在内核数据段的偏移
  .b3:
         push edi
         push esi
         push ecx
  ...
  .b4:
         ...
         mov [es:esi+0x1a],ecx
         shr dword [es:esi+0x1a],12         ;登记0特权级堆栈尺寸到TCB (除以4096，以4KB为单位)
         call sys_routine_seg_sel:allocate_memory ;参数cx -> 需要分配的地址，返回cx -> 分配空间的起始地址
         add eax,ecx                        ;堆栈必须使用高端地址为基地址 eax -> 堆栈段基地址
         mov [es:esi+0x1e],eax              ;登记0特权级堆栈基地址到TCB 
         mov ebx,0xffffe                    ;ebx -> 段长度（界限）
         mov ecx,0x00c09600                 ;ecx -> 4KB粒度，读写，特权级0
         call sys_routine_seg_sel:make_seg_descriptor
         ...
         ;创建用户程序的TSS
         mov ecx,104                        ;ecx -> tss的基本尺寸(申请104字节)
         mov [es:esi+0x12],cx              
         dec word [es:esi+0x12]             ;登记TSS界限值到TCB 
         call sys_routine_seg_sel:allocate_memory
         mov [es:esi+0x14],ecx              ;登记TSS基地址到TCB (ecx -> tss线性基地址)
      
         ;登记基本的TSS表格内容
         mov word [es:ecx+0],0              ;反向链=0（抢一个任务的指针）
      
         mov edx,[es:esi+0x24]              ;edx -> 登记0特权级堆栈初始ESP(tcb中取出)
         ...
         mov dx,[es:esi+0x10]               ;登记任务的LDT选择子（LDT对任务来说不是必须的）
         mov [es:ecx+96],dx                 ;到TSS中
      
         mov dx,[es:esi+0x12]               ;登记任务的I/O位图偏移
         mov [es:ecx+102],dx                ;到TSS中 （值为tss界限段103，表示tss中不存在该区域）
...
append_to_tcb_link:                         ;在TCB链上追加任务控制块
  ...
  .searc:                                   ;链表不为空，顺着TCB链找到最后一个TCB
         mov edx,eax                        ;edx -> TCB表头指针
         mov eax,[es: edx+0x00]             ;eax -> TCB指针所指向的下一个TCB
         or eax,eax               
         jnz .searc
         
         mov [es: edx+0x00],ecx             ;edx -> TCB链表最后一个TCB的表头指针处。指向新生成的TCB
         jmp .retpc
...
start:
  ...
  .b3:
         ...
         mov cx,1_11_0_1100_000_00000B      ;cx -> 特权级3的调用门(3以上的特权级才
                                            ;允许访问)，0个参数(因为用寄存器
                                            ;传递参数，而没有用栈) 
         call sys_routine_seg_sel:make_gate_descriptor      ;edx:eax = 完整的调用门描述符
         call sys_routine_seg_sel:set_up_gdt_descriptor
         ...
         mov ecx,0x46                       ;TCB结构需要46字节的空间
         call sys_routine_seg_sel:allocate_memory
         call append_to_tcb_link            ;将任务控制块追加到TCB链表 
      
         push dword 50                      ;用户程序位于逻辑50扇区（扇区号，参数）
         push ecx                           ;压入任务控制块起始线性地址 （TCB起始线性基地址，参数）
       
         call load_relocate_program
      
         mov ebx,do_status                  ;显示用户程序成功加载
         call sys_routine_seg_sel:put_string
      
         mov eax,mem_0_4_gb_seg_sel
         mov ds,eax
      
         ltr [ecx+0x18]                     ;加载任务状态段TSS(ecx -> TCB线性基地址)
         lldt [ecx+0x10]                    ;加载LDT
      
         mov eax,[ecx+0x44]
         mov ds,eax                         ;ds -> 切换到用户程序头部段 

         ;以下假装是从调用门返回。摹仿处理器压入返回参数 
         push dword [0x08]                  ;调用前的堆栈段选择子
         push dword 0                       ;调用前的esp

         push dword [0x14]                  ;调用前的代码段选择子 （假装压入cs -> 用户代码段选择子）
         push dword [0x10]                  ;调用前的eip
      
         retf                               ;假装从调用门返回（控制转移到特权级为3的用户程序）
