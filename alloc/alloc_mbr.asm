         ...
         ;文件名：c13_mbr.asm
         ;文件说明：硬盘主引导扇区代码 
         ;创建日期：2021-10-28 22:35        ;设置堆栈段和栈指针 
         ...
         mov ax,cs      
         mov ss,ax     
         mov sp,0x7c00                      ;栈初始地址 ss:sp = 0x0000:0x7c00
      
         ;计算GDT所在的逻辑段地址
         mov eax,[cs:pgdt+0x7c00+0x02]      ;GDT的32位物理地址 
         xor edx,edx
         mov ebx,16
         div ebx                            ;分解成16位逻辑地址 eax = 0x000007e0
  ...
  flush:                                  
         ...
         dec eax                            ;已经读了一个扇区，扇区总数减1 (eax = 需要读的扇区数)
   @1:
         or eax,eax                         ;考虑实际长度≤512个字节的情况 
         jz setup                           ;EAX=0 ? （需要读取的扇区数为0，跳过读的部分）
 ...
 setup:
         ...
         mov [esi+0x28],eax                 ;ESI指向GDT的基地址
         mov [esi+0x2c],edx                 ;添加一个描述符
...
make_gdt_descriptor:                     ;构造描述符
         ...
         or edx,ebx                      ;装配段界限的高4位（EBX中的段界限同EDX合并）
      
         or edx,ecx                      ;装配属性（ECX中的段属性与EDX合并）
