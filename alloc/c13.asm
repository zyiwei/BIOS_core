...
start:
         mov eax,ds                          ;ds -> 用户程序头部段
         mov fs,eax                          ;fs -> ds
     
         mov eax,[stack_seg]
         mov ss,eax                          ;ss -> 用户程序自己的栈
         mov esp,0                           ;esp清零
     
         mov eax,[data_seg]
         mov ds,eax                          ;ds -> 用户程序自己的数据段
         ...
         call far [fs:ReadDiskData]          ;段间调用(参数eax -> 逻辑扇区号 ds:ebx -> 存放位置)
     
         mov ebx,message_2
         call far [fs:PrintString]
     
         mov ebx,buffer                      ;显示刚刚从硬盘读出的内容
