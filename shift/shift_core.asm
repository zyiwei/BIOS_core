...
terminate_current_task:                     ;终止当前任务
                                            ...
                                            ;一部分 
         pushfd
         mov edx,[esp]                      ;edx -> 获得EFLAGS寄存器内容
         add esp,4                          ;恢复堆栈指针

         mov eax,core_data_seg_sel
         mov ds,eax                         ;ds -> 核心数据段

         test dx,0100_0000_0000_0000B       ;测试NT位（dx的第14位）
...
start:
  ...
  .b3:
         ...
         ;创建TSS描述符，并安装到GDT中 
         mov eax,ecx                        ;eax -> TSS的起始线性地址
         mov ebx,103                        ;ebx -> 段长度（界限）
         mov ecx,0x00408900                 ;ecx -> TSS描述符属性段，特权级0
         call sys_routine_seg_sel:make_seg_descriptor    ;返回edx:eax -> 8字节TSS描述符
         call sys_routine_seg_sel:set_up_gdt_descriptor  ;返回cx -> TSS描述符选择子
         ...
         call far [es:ecx+0x14]             ;执行任务切换。和上一章不同，任务切
                                            ;换时要恢复TSS内容，所以在创建任务
                                            ;时TSS要填写完整（内存中高2字节为TSS选择子）
                                          
         ;重新加载并切换任务 
         mov ebx,prgman_msg2
         call sys_routine_seg_sel:put_string

         mov ecx,0x46                       ;ecx -> 新任务的TCB空间（46字节）
         call sys_routine_seg_sel:allocate_memory
         call append_to_tcb_link            ;将此TCB添加到TCB链中
