...
;===============================================================================
SECTION  mbr  vstart=0x00007c00             ;为使程序在平坦模型下方便引用内存地址
                                            ;要求段的虚拟地址从0x00007c00开始
         mov ax,cs      
         mov ss,ax
         mov sp,0x7c00
         ;创建保护模式时用到的段描述符
         ;计算GDT所在的逻辑段地址
         mov eax,[cs:pgdt+0x02]             ;eax -> GDT的32位物理地址 
         xor edx,edx                        ;edx清零
         mov ebx,16
         div ebx                            ;分解成16位逻辑地址（eax -> GDT所在的逻辑段地址）

         mov ds,eax                         ;令DS指向该段以进行操作
         mov ebx,edx                        ;ebx -> 段内起始偏移地址 

         ;跳过0#号描述符的槽位 
         ;创建1#描述符，保护模式下的代码段描述符(4GB代码段)
         mov dword [ebx+0x08],0x0000ffff    ;基地址为0，界限0xFFFFF，DPL=00 
         mov dword [ebx+0x0c],0x00cf9800    ;4KB粒度，代码段描述符，向上扩展 

         ;创建2#描述符，保护模式下的数据段和堆栈段描述符(4GB数据段)
         ...
         lgdt [cs: pgdt]                    ;安装GDTR，GDT生效
      
         in al,0x92                         ;南桥芯片内的端口 
         or al,0000_0010B
         out 0x92,al                        ;打开第21根地址线A20
  ...
  flush:                                  
         ...
         mov ds,eax                         ;ds -> 4GB数据段
         mov es,eax
         mov fs,eax
         mov gs,eax
         mov ss,eax                         ;加载堆栈段(4GB)选择子
         mov esp,0x7000                     ;堆栈指针（初始栈从物理地址0x00007000向下推进）
         
         ;以下加载系统核心程序
         mov edi,core_base_address          ;edi -> 内核程序起始线性地址

         mov eax,core_start_sector          ;eax -> 内核起始的逻辑扇区号
         mov ebx,edi                        ;ebx -> 起始地址
         call read_hard_disk_0              ;以下读取程序的起始部分（一个扇区）

         ;以下判断整个程序有多大
         mov eax,[edi]                      ;eax -> 核心程序尺寸
         xor edx,edx
         mov ecx,512                        ;512字节每扇区(ecx -> 内核程序需要的扇区数)
         div ecx

         or edx,edx                         ;edx中为余数
         jnz @1                             ;未除尽，因此结果比实际扇区数少1
         dec eax                            ;已除尽，且已经读了一个扇区，扇区总数减1
   @1:
         or eax,eax                         ;考虑实际长度≤512个字节的情况
         jz pge                             ;EAX=0 ?

         ;读取剩余的扇区
         mov ecx,eax                        ;32位模式下的LOOP使用ECX（ecx -> 扇区数）
         mov eax,core_start_sector          ;eax -> 内核程序起始扇区号
         inc eax                            ;从下一个逻辑扇区接着读
   @2:
   ...
   pge:
         ...
         mov edx,0x00021003                 ;MBR空间有限，后面尽量不使用立即数（页表的物理地址为0x00021000）
  ...
  .b1:       
         ...
         sgdt [pgdt]                        ;读GDTR的6字节到pgdt指向位置
         mov ebx,[pgdt+2]                   ;ebx -> GDT线性基地址
